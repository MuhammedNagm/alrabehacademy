...
@php
                                                                                    // ->where('skipped', false)
    $correctAnswersCount = $quizLogs->children()->where('status', 1)->where('passed', true)->count();
    $wrongAnswersCount = $quizLogs->children()->where('status', 1)->where('passed', false)->where('options', '!=', null)->count();
    $questionsCount = $quiz->questions()->where('question_type', '!=', 'paragraph')->count();
   ...
   $user_passing_grade = ($correctAnswersCount/$questionsCount)*100;
    ...
    if($quizLogs->parent_id && $course){
        ...
        if(!empty($course)){
            ...
            if($is_course_element){
                $quizUrl = route('quizzes.quizPage', ['quiz' => $quiz->hashed_id,'course' => $course->hashed_id]);
            }else{
    ...
    <div class="quiz-results faild">
        ...
        <div class="result-summary">
            <div class="result-field correct">
              <a href="javascript:;" class="preview_quiz_btn"
                   data-url="{{$quizUrl.'?answers_type=correct'}}">
						<span>
            ...
            <div class="result-field wrong">
              <a href="javascript:;" class="preview_quiz_btn"
                   data-url="{{$quizUrl.'?answers_type=wrongs'}}">
						<span>
    ...
    <div class="text-center" style="display: inline-flex;">
        ...
        @if($quizRetake)
            <div id="start_quiz_form" style="margin: 5px">
                ...
                <button type="submit" class="btn btn-primary" id="retake_quiz_btn"> إعادة
                    الاختبار مرة أخرى
