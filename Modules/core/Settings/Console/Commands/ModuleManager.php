<?php namespace Modules\Settings\Console\Commands;
...
class ModuleManager extends Command
{
    ...
    */
     ...
     * @var string
     */
    protected $description = 'Modules Module Manager';
    ...
    public function handle()
    {
        ...
        if (!$this->action || !in_array($this->action, ['download', 'update', 'enable', 'disable', 'install', 'uninstall'])) {
    ...
    private
    function updateTheme($theme_name, $remote_updates, $themes)
    {
        ...
        if (array_key_exists($theme_name, $remote_updates)) {
            ...
            if (version_compare($remote_updates[$theme_name]['version'], $installed_theme->version, '>') || $this->force) {
    ...
    private
    function updateModule($module_code, $remote_updates)
    {
    ...
    private function checkForUpdates($type, $installed)
    {
        ...
        if ($check_updates_result['message']) {
