...
use Modules\Theme\ThemeManifest;
...
class Themes
{
    ...
    public function set($themeName)
    {
        ...
        Event::fire('modules.laravel-theme.change', $theme);
    ...
    public function formatThemeActions($theme, $type, $remote_updates)
    {
        ...
        if ($theme->name != config('themes.modules_' . $type) && $theme->name != \Settings::get('active_' . $type . '_theme')) {
            $actions .= ModulesForm::link(url($resource_url . '/uninstall/' . $theme->name), 'Theme::labels.theme.theme_uninstall', [
        ...
        if ($theme->name != \Settings::get('active_' . $type . '_theme')) {
            $actions .= $spacer . ModulesForm::link(url($resource_url . '/activate/' . $type . '/' . $theme->name), 'Theme::labels.theme.theme_activate', [
        ...
        } elseif (config('themes.modules_' . $type) == \Settings::get('active_' . $type . '_theme')) {
            $actions .= $spacer . '<b><i class="fa fa-check-circle text-success"></i> Active</b>';
        } else {
            $actions .= $spacer . ModulesForm::link(url($resource_url . '/deactivate/' . $type . '/' . $theme->name), 'Theme::labels.theme.theme_deactivate', [
        ...
        if ($theme->name == \Settings::get('active_' . $type . '_theme') && $type == 'frontend') {
            $actions .= $spacer . ModulesForm::link(url($resource_url . '/import-demo/' . $theme->name), 'Theme::labels.theme.theme_import_demo', [
        ...
        if (isset($remote_updates[$theme->name]) && version_compare($remote_updates[$theme->name]['version'], $theme->version, '>')) {
            $actions .= $spacer . ModulesForm::link(url($resource_url . '/download-update/' . $theme->name), trans('Theme::labels.theme.theme_new', ['version' => $remote_updates[$theme->name]['version']]), [
                    'class' => 'btn btn-primary btn-xs',
                    'data' => [
                        'action' => 'post',
    ...
    public function installTheme($filename, $update_if_exist)
    {
        ...
        \Zipper::make($themeTempPath)->extractTo($this->tempPath);
        ...
        if ($this->theme_installed($themeName) && !$update_if_exist) {
