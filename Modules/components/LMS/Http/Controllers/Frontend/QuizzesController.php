...
class QuizzesController extends PublicBaseController
{
    ...
    public function __construct()
    {
        ...
        //        \LMS::setQuizParentSession(\Request::fullUrl());
    }
    ...
    public function index()
    {
        ...
        $quizzes = Quiz::where([['status', '=', 1], ['is_sub_quiz', '<', 1], ['show_in_plan', '<', '1']])->paginate(10);
    ...
    public function show_questions($hashed_id)
    {
        ...
        return view('quizzes.questions')->with(compact(
            'quiz',
            'relatedQuizzes',
            'subscriptionStatus'
    ...
    public function quizPage(Request $request, $quiz_hashed_id, $course_hashed_id = null, $retake_quiz = null, $parent_name = null, $parent_id = null, $subparent_name = null, $subparent_id = null)
    {
        $handelQuizRequest = $this->handelQuizRequest($parent_name, $parent_id, $subparent_name, $subparent_id);

        if (!$handelQuizRequest['success']) {
            return abort(404);
        }

        $quizCourse = $handelQuizRequest['course'];
        $quizPlan = $handelQuizRequest['plan'];
        ...
        if ($course_id) {
            ...
            $courseSections = $course->sections()->where('parent_id', null)->orderBy('order', 'asc')->with('children')->with('lessons')->with('quizzes')->get();
            ...
            if (!$request->ajax()) {
                $subscriptionStatus = \Subscriptions::check_subscription($parentModuleArray);
            } else {
                $subscriptionStatus = ['success' => true, 'status' =>  1, 'data' => null, 'message' => 'Ajax loaded']; //@transM
        ...
        $delayedQuestions = [];
        if ($enroll_status['itemLog']) {
            $delayedQuestions = $enroll_status['itemLog']->children()->where('lms_loggable_type', 'question')->where('delayed', '>', 0)->pluck('lms_loggable_id')->toArray();
        }

        // if(isset($not_show_delayed) && $not_show_delayed){

        if ($request->ajax() && $request->get('answers_type')) {

            Session::put('answers_type', $request->get('answers_type'));
        } elseif ($request->ajax() && $request->get('show_answers')) {
            if (Session::has('answers_type')) {
                Session::forget('answers_type');
            }
        } else {

            if (!$request->ajax() && Session::has('answers_type')) {
                Session::forget('answers_type');
            }
        }


        if (Session::has('answers_type') && $request->ajax()) {
            $questions_type = Session::get('answers_type');
            $questions = $quiz->questions()
                ->whereHas('studentLogs', function ($q) use ($questions_type) {
                    $this->get_logged_questions_by_status($questions_type, $q);
                })
                ->where('status', 1)
                ->orderBy('id', 'asc')
                ->whereNotIn('lms_questions.id', $delayedQuestions)
                ->where('question_type', '!=', 'paragraph')->orderBy('parent_id', 'asc')
            ...
            $questionsList = $quiz->questions()
                ...
                ->orderBy('id', 'asc')
                ->whereNotIn('lms_questions.id', $delayedQuestions)
                ->where('question_type', '!=', 'paragraph')->orderBy('parent_id', 'asc')->orderBy('pivot_order', 'asc')->select('lms_questions.content', 'lms_questions.id', 'lms_questions.parent_id')->get()->pluck('content', 'id')->toArray();
        } else {
            $hasParagraphsCount = $quiz->questions()->whereIn('lms_questions.question_type', ['paragraph'])->count();
            $questions = $quiz->questions()
                ->where('status', 1);
            if ($hasParagraphsCount <= 0) {
                $questions = $questions->orderBy('lms_questions.id', 'asc');
            }
            $questions = $questions->whereNotIn('lms_questions.id', $delayedQuestions)
                ->whereNotIn('lms_questions.question_type', ['paragraph'])
                ->orderBy('lms_questions.id', 'asc')
                ->orderBy('lms_questions.parent_id', 'asc')
                ->orderBy('pivot_order', 'asc')
                ->paginate($quiz->question_per_page ?: 1);

            $questionsList = $quiz->questions()
                ->where('status', 1);
            if ($hasParagraphsCount <= 0) {
                $questionsList = $questionsList->orderBy('lms_questions.id', 'asc');
            }
            $questionsList = $questionsList->whereNotIn('lms_questions.id', $delayedQuestions)
                ->whereNotIn('lms_questions.question_type', ['paragraph'])
                ->orderBy('lms_questions.id', 'asc')
                ->orderBy('lms_questions.parent_id', 'asc')
                ->orderBy('pivot_order', 'asc')->get()->pluck('content', 'id')->toArray();
        }
            ...
            if ($quiz->duration > 0) {
                ...
                if ($finishTime > Carbon::now()) {
                    ...
                    if ($request->ajax() && $request->get('page')) {
                        ...
                        //     $quizLogs->update(['current_page' => \Request::get('page')]); //2
        ...
        if ($request->ajax() && ($request->get('answers_type') || $request->get('show_answers') || $request->get('page'))) {
    ...
    public function get_logged_questions_by_status($status, $quiry)
    {

        switch ($status) {
            case 'wrongs':
                return $quiry->where('user_id', user()->id)->where('status', 1)->where('passed', false)->where('options', '!=', null);
                break;

            case 'correct':
                return  $quiry->where('user_id', user()->id)->where('status', 1)->where('passed', true)->where('skipped', false);
                break;

            default:
                return  $quiry->where('user_id', user()->id)->where('status', 0)->where('options', '!=', null)->orWhere('skipped', true);
                break;
        }
    }
    ...
    public function previewQuestion(Request $request, $quiz_hashed_id, $question_hashed_id)
    {
        ...
        return view('quizzes.question_preview')->with(compact('quiz', 'question'))->render();
    }
    ...
    public function retakeQuiz(Request $request, $quiz_hashed_id)
    {
        ...
        // }
        if ($quiz->retake_count && $logsCount >= $quiz->retake_count) {
            $message = __('LMS::messages.cannot_retake_quiz');
            return view('partials.quiz_body.templates.error')->with(compact('message'));;
        }
        ...
        StudentLogs::Create([
            ...
            'parent_id' => $quizLogs->parent_id

        ]);


        return redirect()->route('quizzes.quizPage', ['quiz' => $quiz->hashed_id, 'course' => $request->get('course')]);
    }
    ...
    public function favourite_questions(Request $request, $quiz_hashed_id, $logs_hashed_id, $course_hashed_id = null)
    {
        ...
        $questionsList = $quiz->questions()
            ...
            ->whereIn('lms_questions.id', $favouriteQuestionsIds)
            ->where('question_type', '!=', 'paragraph')
            ->orderBy('parent_id', 'asc')->orderBy('pivot_order', 'asc')->select('lms_questions.content', 'lms_questions.id', 'lms_questions.parent_id')->get()->pluck('content', 'id')->toArray();
    ...
    public function getAskTeacherModal(Request $request, $quiz_hashed_id, $question_hashed_id)
    {
        ...
        $teachers = \Modules\User\Models\User::where('user_type', 'teacher')->select('name', 'id', 'job_title')->get();
        if (!$teachers->count()) {
            $message = 'نأسف .. لا يوجد معلمين متاحين حالياً للتواصل معهم.';
            return view('partials.quiz_body.templates.error')->with(compact('message'));
        }
        return view('partials.quiz_body.components.ask_teacher_form')->with(compact('quiz', 'question', 'teachers'))->render();
    }
    ...
    public function handelQuizRequest($parent_name = null, $parent_id = null, $subparent_name = null, $subparent_id = null)
    {
        $returnArray = ['success' => true, 'plan' => null, 'course' => null];
        if ($parent_name == 'packages') {
            $plan_id = hashids_decode($parent_id);
            $plan = Plan::find($plan_id);
            if (!$plan) {
                return ['success' => false];
            }
            $returnArray['plan'] = $plan;
            if ($subparent_name == 'courses') {
                $course_id = hashids_decode($subparent_id);
                $course = Course::find($course_id);
                if (!$course) {
                    return ['success' => false];
                }
            }
            $returnArray['course'] = $course;
        } elseif ($parent_name == 'courses') {
            $course_id = hashids_decode($parent_id);
            $course = Course::find($course_id);
            if (!$course) {
                return ['success' => false];
            }
    ...
    public function loadInIfram(Request $request, $quiz_hashed_id, $course_hashed_id = null, $retake_quiz = null)
    {
        ...
        $enroll_status = \Logs::enroll_status($moduleArray);
        $delayedQuestions = [];
        if (countData($enroll_status['itemLog'])) {
            $delayedQuestions = $enroll_status['itemLog']->children()->where('lms_loggable_type', 'question')->where('delayed', '>', 0)->pluck('lms_loggable_id')->toArray();
        }
        ...
        return view('partials.quiz_body.embeded_quiz')->with(compact('quizLogs', 'quiz', 'course', 'quizTemplate', 'page_title', 'questions', 'subscriptionStatus', 'questionsList', 'remove_page'));
    }
}
