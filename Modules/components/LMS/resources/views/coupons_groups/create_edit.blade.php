...
@section('content')
    @parent
    <div class="row">
         {!! Form::model($coupon_group, ['url' => url($resource_url.'/'.$coupon_group->hashed_id),'method'=>$coupon_group->exists?'PUT':'POST','files'=>true,'class'=>'ajax-form']) !!}
                ...
                <div class="row">
                    <div class="col-md-12">
                     ...
                     <br>
                      ...
                      {!! ModulesForm::radio('start_with_first_use','يبدأ مع اول استخدام',false, trans('LMS::attributes.main.yes_no'),$coupon_group->exists?(int)$coupon_group->start_with_first_use:0) !!}
                      {!! ModulesForm::number('duration_in_days','المدة بالايام (عند تفعيل الخيار السابق)',false,$coupon_group->exists?(int)$coupon_group->duration_in_days:0, array_merge(['step'=>1,'min'=>1,'max'=>999999])) !!}
                    </div>
                </div>
