...
if (!function_exists('hashids_encode')) {
    ...
    {
        return \Modules\Foundation\Facades\Hashids::encode($id);
    }
...
if (!function_exists('hashids_decode')) {
    ...
    {
        $decoded_value = \Modules\Foundation\Facades\Hashids::decode($value);
...
if (!function_exists('formatStatusAsLabels')) {
    function formatStatusAsLabels($status, $customConfig = [])
    {
        ...
        $default_translation_key = (is_numeric($status) || is_bool($status)) ? 'Modules::attributes.status_options_boolean.' : 'Modules::attributes.status_options.';

        $defaultLevel = $is_active ? 'success' : ($is_inactive ? 'warning' : 'default');
        ...
        $response = "<span class=\"label label-{$level}\">{$icon} {$text}</span>";
...
if (!function_exists('getUserByHash')) {
    ...
    {
        $user = Modules\User\Models\User::findByHash($user_hashed_id);
        return $user;
    }
}
