...
{{--    {{ dd($dataTable->bulkActions())}}
 --}}    @if(!empty($dataTable->bulkActions()))
    ...
    @unless(isset($hideCreate))
        {!! ModulesForm::link(url($resource_url.'/create'), trans('Modules::labels.create'),['class'=>'btn btn-success']) !!}
    @endunless
    @stack('after_actions_btns')
...
@section('js')
    ...
    <script type="text/javascript">
           ...
           var table = $('{{'#'.$dataTable->getTableAttributes()['id']}}').DataTable();
