 ...
 <div class="panel box box-success parent_section_row" id="sectionItemRow_{{$section->id}}">
                  ...
                  <div class="box-header with-border">
                    ...
                    if($section->parent_id){
                      $parentDivId = '#section-items-list-group-'.$section->hashed_id;
                    }else{
                      $parentDivId = '#course-items-list-group';
                    }

                    @endphp

                  	<a data-toggle="collapse" data-parent="{{$parentDivId}}" href="#collapse_section_{{$section->id}}">
               ...
               <ul id="list-group-section_{{$section->id}}">
                @php
               $section_children =  $section->children()->orderBy('order', 'asc')->get();

                @endphp
                @if($section_children->count())
          <div class="box box-solid">
            <!-- /.box-header -->
            <div class="box-body">
              <div class="box-group" id="section-items-list-group-{{$section->hashed_id}}">
                 @foreach($section_children as $child)
                  @include('LMS::courses.partials.selected_items', ['section' => $child])
                 @endforeach 
               </div>
             </div>
           </div>
   ...
   <div class="input-group ajax_submit_inputs" style="width: 100%">
                ...
                <span class="input-group-btn">
                 <div class="btn-group">
                  ...
                  <ul class="dropdown-menu" role="menu">
                    <li><a href="javascript:;" class="add_section_item" data-section-id="{{$section->id}}" data-item-type="section">{{ __('LMS::attributes.courses.add_section')}}</a></li>
