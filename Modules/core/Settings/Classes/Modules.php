...
namespace Modules\Settings\Classes;
...
/**
 * Class Modules
 * @package Modules\Settings\Classes
 */
class Modules
{
    ...
    {
        $modules = Cache::remember('module_settings_list', 10, function () {
            $modules = [];
            ...
            foreach ($modules as $row) {
                ...
                if (!$module) {
                    ...
                    $params = [
                        'code' => $code,
                        'load_order' => array_get($data, 'load_order', 0),
                        'type' => $type,
                        'folder' => $folder,
                        'provider' => $provider
                    ];
                    ...
                    if ($type == 'core' || $type == 'module') {
                        $params['enabled'] = 1;
                        $params['installed'] = 1;
                        $params['installed_version'] = array_get($data, 'version');
                    }
    ...
    public function modulesVersionCompareMessage($module, $condition)
    {
        ...
        switch ($where) {
            case '^':
                $message = trans('Settings::labels.message.version_must_higher', ['name' => $module->code, 'version' => substr($condition, 1)]);
                break;
            case '~':
                $message = trans('Settings::labels.message.version_must_lower_or_equal', ['name' => $module->code, 'version' => substr($condition, 1)]);
                break;
            default:
                $message = trans('Settings::labels.message.version_must_equal', ['name' => $module->code, 'version' => $where]);
        }
        ...
        foreach ($enabled_modules as $enabled_module) {
            ...
            foreach ($required as $moduleCode => $version) {
                if ($moduleCode == $module->code) {
                    throw new \Exception(trans('Settings::exception.module.module_required', ['required_module' => $module->code, 'module' => $enabled_module->code]));
                }
    ...
    public function getModuleAction($module, $remote_updates = [])
    {
        ...
        if ($module->installed && $module->type != 'core') {
            ...
            if (isset($result['error'])) {
                $actions .= '<span class="label label-warning"><i class="fa fa-info-circle"></i> ' . $result['error'] . '</span>';
            } else {
                $actions .= $this->formatAction($module, 'install');
            }
        }

        if ($module->installed && !is_null($module->installed_version) && version_compare($module->version, $module->installed_version, '>')) {
            $actions .= $this->formatAction($module, 'update');
        }
        if (array_key_exists($module->code, $remote_updates) && version_compare($remote_updates[$module->code]['version'], $module->version, '>')) {
     ...
     * @throws \Exception
     */
    function executeModuleBatches($moduleCode, $batches)
    {
     ...
     * @throws \Exception
     */
    public function normalizeDatabaseModule($module)
    {
    ...
    public function install($module)
    {
        try {
            ...
            $namespace = trim(str_replace('\\\\', '\\', $moduleSettings->namespace), '\\');

            if (property_exists($moduleSettings, 'provider')) {
                $provider = $namespace . '\\' . $moduleSettings->provider;
            ...
            $install_provider = $namespace . '\Providers\InstallModuleServiceProvider';
            ...
            throw new \Exception(trans('Settings::exception.module.install_module_failed', ['message_exception' => $e->getMessage()]));
    ...
    public function update($module)
    {
        ...
        $update_provider = $namespace . '\Providers\UpdateModuleServiceProvider';
    ...
    public function downloadRemote($module)
    {
        ...
        try {
            ...
            \Zipper::make($downloaded_file)->extractTo(base_path($module_paths[$moduleSettings->type]), array($moduleSettings->folder), 1);
        } catch (\Exception $e) {
            if ($module_backup_file) {
                $this->restore($module_backup_file, $module_path);
            }
            throw new \Exception(trans('Settings::exception.module.download_module_failed', ['message_exception' => $e->getMessage()]));
        }
        ...
        \Cache::put('modules_remote_updates', $remote_updates, 1440);
...

    {
        ...
        try {
            ...
            \Zipper::make($downloaded_file)->extractTo(base_path($path));
        } catch (\Exception $e) {
    ...
    public function download($module_key, $module_license_key, $tempFolder = null)
    {
        ...
        $url = url('');
        $plugin_download_url = "{$updater_url}?action=downloadPlugin&license_key={$license_key}&plugin_code={$module_key}&plugin_license_key=" . $module_license_key . '&domain=' . $url;
    ...
    {
        try {
            \Zipper::make($module_backup_file)->extractTo($module_path);
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
        return true;
...
//        }
        ...
        $uninstall_provider = $namespace . '\Providers\UninstallModuleServiceProvider';
    ...
    public function checkForUpdates($request, $modules = [], $cacheKey)
    {
        ...
        if (isset($request['check-for-updates'])) {
            ...
            //     \Cache::put($cacheKey, $remote_updates, 1440);
        ...
        // return ['remote_updates' => $remote_updates, 'message' => $check_updates_result['message'], 'status' => $check_updates_result['status'], 'has_updates' => $check_updates_result['has_updates'] ?? false];
        return ['remote_updates' => '', 'message' => ' no updates', 'status' => false, 'has_updates' => false];
    }
    ...
    {
        $module_updates_count = Cache::remember('module_updates_count', 60, function () {
            ...
            try {
                $check_updates_result = \Modules::checkForUpdates(['check-for-updates' => true], $installed_modules, 'modules_remote_updates');
