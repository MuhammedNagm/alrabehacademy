<?php

namespace Modules\User\Communication\Models;

use Modules\Foundation\Models\BaseModel;
use Spatie\Permission\Traits\HasRoles;


/**
 * Class NotificationTemplate
 * @package Modules\User\Communication\Models
 * @property integer id
 * @property string name
 * @property string title
 * @property array body
 * @property array via
 * @property array extras
 * @property array forced_channels
 */
class NotificationTemplate extends BaseModel
{

    use HasRoles; // for morph relation with model_has_roles table

    /**
     *  Model configuration.
     * @var string
     */
    public $config = 'notifications.models.notification_template';

//    protected static $logAttributes = ['value'];

    protected $guarded = ['id'];

    protected $casts = [
        'body' => 'array',
        'extras' => 'array',
        'via' => 'array',
    ];

    public function getForcedChannelsAttribute()
    {
        return $this->via ? array_diff($this->via, ['user_preferences']) : [];
    }
}
