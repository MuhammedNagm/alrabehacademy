...
@section('content_header')
    @component('components.content_header')
        @slot('page_title')
            <span style="color: #673ab7;">{{'اختبار: '.$quiz->title}}</span> -{{ $title_singular }}
        @endslot
        @slot('breadcrumb')
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            {!! Form::model($question, ['url' => trim(url($resource_url.'/'.$question->hashed_id),'/').'?paragraph='.\Request::get('paragraph'),'method'=>$question->exists?'PUT':'POST','files'=>true,'class'=>'ajax-form']) !!}
            <div class="row">
                <div class="col-md-8">
 ...
 {!! ModulesForm::text('title','LMS::attributes.main.title',true,$question->exists?$question->title:$question_title) !!}
                ...
                <div class="col-md-4">
                    ...
                    $qQuizList = [];
                    // if($question->exisits){
                    //    $qQuizList = $question->quizzes()->pluck('title', 'id')->toArray();
                    // }
                     $qQuizList[]=$quiz?$quiz->id:0;
                    @endphp
                    @component('components.box')
                        {!! ModulesForm::text('order','LMS::attributes.main.order',false,$question_order) !!}

                        {!! ModulesForm::select('quizzes[]','LMS::attributes.main.quizzes', \LMS::getAllQuizzesList(),false,$qQuizList,['multiple'=>false], 'select2') !!}
