        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-9 course-wrap">
                    ...
                    @php
                                $moduleArray = ['module' => 'course','module_id' => $course->id, 'user' => $authUser];
            ...
            <div class="message message-danger alert alert-danger" role="alert"role="alert">
                    <i class="fa"></i> <p><strong>تنبيه!...</strong>  يجب تفعيل الاشتراك بهذه الدورة التدريبية لتتمكن من عرض المحتوى.  </p>
                    <a href="{{url('/info-payment')}}" target="_blank">عرض طريقة تفعيل الإشتراك</a></div>
            @endif
...
{{--                                 @if($coursePrice > 0)
                               ...
                               <button type="button"  data-toggle="modal" data-target="{{'#subCourse_'.$course->hashed_id}}" class="colored-btn-red" @if(($planned['success'] && $planned['status'] < 1) || ($subscriptionStatus['success'])) disabled='' style="background-color: #949596;" @endif >@lang('developnet-lms::labels.spans.span_subscribe_now')</button> 

                            @else
                            @if($course->live_class_status > 0)
                                <a href="{{route('courses.live_class', ['course_id' => $course->hashed_id])}}" class="btn btn-dark" 
                                   > عرض القاعة  <i class="fa fa-circle" style="color: red;"></i></a>
                                   @endif
                    ...
                    @if($course->preview_video)
                        ...
                        @include('components.embeded_media', ['embeded' => $course->preview_video, 'height' => '500px;'])
                    @else
                        @if($course->thumbnail)
...
@endsection
 ...
 @push('child_after_content')
   ...
   @php
 $is_free = true;
 if($course->coupons()->count()){
     $is_free = false;
 }
 @endphp

@include('components.subscribe_modal', ['modal_id' => 'subCourse_'.$course->hashed_id,'subscriptionStatus' => $subscriptionStatus, 'planned' => $planned, 'module_data' => $course, 'module' => 'course', 'is_free' => $is_free])
