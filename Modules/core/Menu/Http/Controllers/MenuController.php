...
class MenuController extends BaseController
{
    ...
    public function index(MenuRequest $request, $menu_key = 'sidebar')
    {
        try {
            ...
            $this->setViewSharedData(['title_singular' => trans('Modules::labels.update_title', ['title' => $root->name])]);
    ...
    public function store(MenuRequest $request)
    {
        try {
            ...
            $message = ['level' => 'success', 'message' => trans('Modules::messages.success.created', ['item' => $this->title_singular])];
        } catch (\Exception $exception) {
    ...
    {
        $this->setViewSharedData(['title_singular' => trans('Modules::labels.update_title', ['title' => $menu->name])]);
    ...
    public function update(MenuRequest $request, Menu $menu)
    {
        try {
            ...
            $message = ['level' => 'success', 'message' => trans('Modules::messages.success.updated', ['item' => $this->title_singular])];
        } catch (\Exception $exception) {
    ...
    public function toggleStatus(Request $request, Menu $menu)
    {
        try {
            if (!user()->can('update', $menu)) {
                abort(403, trans('Modules::exceptions.403'));
            }
    ...
    protected function buildTree($id, $json_tree, &$tree)
    {
        foreach ($json_tree as $node) {
            $tree[$id][] = array_get($node, 'id');

            if (isset($node['children'])) {
                $this->buildTree(array_get($node, 'id'), $node['children'], $tree);
            }
        }
    }
}
