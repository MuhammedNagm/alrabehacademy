...
class LMS
{
    ...
    public function setQuizParentSession($url)
    {
        // dd(session()->get('quiz_parent_items'));
        $parts = explode('/', $url, 7);
        if ($parts[3] == 'packages') {
            if ($parts[5] == 'courses') {
                $package = $parts[4];
        ...
        if (session()->has('quiz_parent_items')) {
            session()->forget('quiz_parent_items');
        }

        session()->put('quiz_parent_items', ['plan' => $package, 'course' => $course]);


    }

    public function getQuizPerfixUrl($url)
    {
        $parts = explode('/', $url, 7);
        if ($parts[3] == 'packages') {
            if ($parts[5] == 'courses') {
                $route = 'packages.courses.';
                $prefUrl = '/' . $parts[3] . '/' . $parts[4] . '/' . $parts[5] . '/' . $parts[6];
                return ['route' => $route, 'prefUrl' => $prefUrl];

            }

            $route = 'packages.';
            $prefUrl = '/' . $parts[3] . '/' . $parts[4];
        ...
        return ['route' => null, 'prefUrl' => null];
    }
    ...
    {
        $questions = Question::whereNotNull('id')->where('status', '>', 0)->where('question_type', 'paragraph');
    ...
    public function getQuizzesList($objects = false, $status = null)
    {
        ...
        $quizzes = $quizzes->where('status', 1)->where('show_in_plan', 0)->where('is_sub_quiz', '!=', 1)->pluck('title', 'id');

        return $quizzes;
    }

    public function getAllQuizzesList($objects = false, $status = null)
    {
        $quizzes = Quiz::whereNotNull('id');
        $quizzes = $quizzes->where('status', 1)->where('is_sub_quiz', '!=', 1)->pluck('title', 'id');
    ...
    public function getAllCoursesList($objects = false, $status = null)
    {
        $courses = Course::whereNotNull('id');
        $courses = $courses->where('status', 1)->pluck('title', 'id');
    ...
    {
        $courses = Course::whereNotNull('id');
        $courses = $courses->where('status', 1)->where('show_in_plan', 0)->pluck('title', 'id');
    ...
    {
        $courses = Course::where('status', true)->where('show_in_plan', '<', 1)->orderBy('created_at', 'desc')->take(8);
        return $courses;
    }
...
// ['is_standlone','=','1'],
    ...
    {
        $quizzes = Quiz::where([['status', '=', '1'], ['is_sub_quiz', '<', 1], ['show_in_plan', '<', 1]])->orderBy('created_at', 'desc')->take(4);
        return $quizzes;
    }

    public function leatestPlans()
    {
        $plans = Plan::where([['status', '=', '1']])->orderBy('created_at', 'des')->take(6);
        return $plans;
    }
