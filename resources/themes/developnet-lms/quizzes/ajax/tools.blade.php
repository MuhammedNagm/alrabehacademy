   ...
   @if ($paginator->hasPages()) {{-- if has pages (pagination) --}}
        @if ($paginator->onFirstPage())
          ...
          @endif
          <li class="page-item"><button class="btn btn-default prev" style="margin-left: 10px; margin-right: 10px;"><span>({{$paginator->currentPage().' / '.$paginator->lastPage()}})</span> </button></li>
...
{{--              @if (is_array($element))
        ...
        @else
                ...
                @if($delayed_questions->count())
           <li class="page-item"><a  type="button" class="btn btn-dark" href="{{route('quizzes.delayed_questions', ['quiz' => $quiz->hashed_id, 'quiz_logs' => $quizLogs->hashed_id])}}"> عرض المؤجلة  <span class="badge badge-danger"> {{$delayed_questions->count()}}</span></a></li>
           @else
