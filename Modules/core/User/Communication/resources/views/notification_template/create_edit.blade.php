    ...
    <div class="row">
        <div class="col-md-12">
            @component('components.box')
                {!! ModulesForm::openForm($notification_template) !!}
                <div class="row">
                    <div class="col-md-4">
                        {!! ModulesForm::text('name', 'Notification::attributes.notification_template.name', false, $notification_template->friendly_name, ['readonly'=>'readonly', 'disabled']) !!}
                        {!! ModulesForm::text('title', 'Notification::attributes.notification_template.title' , true, $notification_template->title, ['help_text' => 'Notification::attributes.notification_template.title_help' ]) !!}
                        {!! ModulesForm::select("extras[bcc_roles][]",  'Notification::attributes.notification_template.bcc_roles' , \Roles::getRolesList(['all' => true]), false, $notification_template->extras['bcc_roles']??[],
                         ['multiple', 'help_text' => 'Notification::attributes.notification_template.bcc_roles_help'], 'select2') !!}

                        {!! ModulesForm::select("extras[bcc_users][]",  'Notification::attributes.notification_template.bcc_users' , [], false, $notification_template->extras['bcc_users']??[],
                    ...
                    <div class="col-md-4">
                        {!! ModulesForm::checkboxes('via[]', 'Notification::attributes.notification_template.via' ,true,
                        ...
                        {!! ModulesForm::select("role_ids[]",  'Notification::attributes.notification_template.roles' , \Roles::getRolesList(['all' => true]), false, $notification_template->roles,
                        ...
                        <div class="tab-content">
                            @foreach(config('notification.supported_channels') as $channelKey => $channelName)
                                <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="{{$channelKey}}">
                                    <div class="row">
                                        <div class="col-md-12">
                                            {!! ModulesForm::textarea("body[$channelKey]", $channelName, true,
                ...
                {!! ModulesForm::closeForm($notification_template) !!}
            @endcomponent
        </div>
    </div>
@endsection
