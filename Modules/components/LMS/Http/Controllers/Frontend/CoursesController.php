...
class CoursesController extends PublicBaseController
{
    ...
    function index()
    {
        ...
        // all courses
        $courses = Course::where('status', 1)->where('show_in_plan',0)->paginate(10);
    ...
    function live_class($hashed_id)
    {
        ...
        $moduleArray = ['module' => 'course','module_id' => $course->id, 'user' => $user];
        ...
        $item = [
            'title'            => $course->title.' | درس مباشر',
            'meta_description' => str_limit(strip_tags($course->meta_description), 500),
            'url'              => route('courses.live_class', $course->hashed_id),
            'type'             => 'course',
        ...
        return view('courses.live_class')->with(compact('course'
        ));
    ...
    function showLesson($course_hashed_id, $lesson_hashed_id)
    {
        ...
        if ($subscriptionStatus['success'] && $subscriptionStatus['status'] > 0) {
            ...
            $pagination_lessons = false;
             if ($pagination_lessons) {
