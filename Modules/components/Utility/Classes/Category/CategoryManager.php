...
namespace Modules\Components\Utility\Classes\Category;

use Modules\Foundation\Facades\ModulesForm;
use Modules\Components\Utility\Models\Category\Attribute;
use Modules\Components\Utility\Models\Category\AttributeOption;
...
class CategoryManager
{
    ...
    public function getCategoriesList($module = null, $parentsOnly = false, $objects = false, $status = null, $except = [], $featured = false)
    {
    ...
    public function renderAttribute($field, $product = null, $attributes = [])
    {
        ...
        switch ($field->type) {
            ...
            case 'text':
            case 'textarea':
                $input = ModulesForm::{$field->type}('options[' . $field->id . ']', $field->label, $asFilter ? false : $field->required, $value, $attributes);
                break;
            case 'checkbox':
                $input = ModulesForm::{$field->type}('options[' . $field->id . ']', $field->label, $value, 1, $attributes);
                break;
            case 'radio':
                $input = ModulesForm::{$field->type}('options[' . $field->id . ']', $field->label, $asFilter ? false : $field->required, $field->options->pluck('option_display', 'id')->toArray(), $value, $attributes);
                break;
            case 'select':
                $input = ModulesForm::{$field->type}('options[' . $field->id . ']', $field->label, $field->options->pluck('option_display', 'id')->toArray(), $asFilter ? false : $field->required, $value, $attributes, 'select2');
                break;
            case 'multi_values':
                $attributes = array_merge(['class' => 'select2-normal', 'multiple' => true], $attributes);
                $input = ModulesForm::select('options[' . $field->id . '][]', $field->label, $field->options->pluck('option_display', 'id')->toArray(), $asFilter ? false : $field->required, $value, $attributes, 'select2');
                break;
        }
