    ...
    if($quiz->duration > 0 && \LMS::getRemainTime($quiz->duration, $quizLogs->created_at) < ($quiz->duration * (1/3))){
                ...
                <div class="quiz-questions" id="question_{{$question->hashed_id}}">
                    ...
                    <div class="form-group checkbox ck-button answer-row" data-question="{{$question->hashed_id}}">
                        <label>
                            ...
                            <input type="checkbox" disabled="" @if(in_array($answer->hashed_id, $userAnswers)) class="form-control" checked="" @endif hidden > <span class="main_span {{$answer_mouse_class}}" @if(in_array($answer->hashed_id, $userWrongAnswers)) style="background-color: red !important; color: #fff;" @elseif(in_array($answer->hashed_id, $correctAnswersArray)) style="background-color: #28a745 !important; color: #fff;"@endif> @if($quiz->numbers_in_arabic) {!! convertToArabicNumbers($answer->title) !!} @else {!! $answer->title !!} @endif</span>
                            @else
                    ...
                    <div style="display: flex;">
                        ...
                        @if($quiz->duration < 1)
                            ...
                            <span style="color: #f10b21 !important"> جوابك خطأ</span>

                            @else

                            <span style="color: #f10b21 !important" class="check-ans-msg"> جوابك خطأ ... أعد المحاولة مرة اخرى. </span>
                    ...
                    <div class="question-meta">
                        ...
                        <div style="background-color: {{($question->question_explanation && $questionLogs->preview > 0)?'#28a745 !important' : '#bcbcbc'}};">
                        ...
                        @endif
                        <div style="background-color: {{($quiz->duration > 0)?'#bcbcbc ; cursor: no-drop;': '#007bff !important; cursor: pointer;'}}">
        ...
        <script>
            $(function() {
                ...
                $('.collapse-btn').on("click", function() {
                    ...
                    localStorage.setItem("question_collapses", JSON.stringify({
                        target: target,
                        status: status
                    }));
                });
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('.embededWrapper').find('iframe').attr('src', '{{$ifram_url}}').css({
                    "max-height": height,
                    "min-height": height,
                    "min-width": '100%'
                });
