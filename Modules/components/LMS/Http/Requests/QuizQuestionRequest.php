    ...
    public function rules()
    {
        ...
        if ($this->isUpdate() || $this->isStore()) {
            $rules = array_merge($rules, [
             ...
             $parQuestion = Question::where('title',$this->get('title'))->where('question_type', 'paragraph')->first();
              if($parQuestion){

            
            if ($this->isStore()) {
            $rules = array_merge($rules, [
                'title' => 'required|max:191|unique:lms_questions,title'
            ]);
        }
        ...
        if ($this->isUpdate()) {
            ...
            $rules = array_merge($rules, [
                'title' => 'required|max:191|unique:lms_questions,title,' . $question->id,
            ]);
        }
