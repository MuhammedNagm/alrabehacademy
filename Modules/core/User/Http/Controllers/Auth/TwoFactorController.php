...
class TwoFactorController extends AuthBaseController
{
    ...
    {
        $this->modules_middleware_except = ['showTokenForm', 'validateTokenForm'];
        parent::__construct();
    }
    ...
    public function showTokenForm()
    {
        if (session('authy:auth:id')) {
            try {
                ...
                flash(trans('User::exception.invalid_send_token_user'), 'error');
    ...
    public function validateTokenForm(Request $request)
    {
        ...
        if (TwoFactorAuth::tokenIsValid($user, $request->token)) {
            ...
            flash(trans('User::exception.invalid_two_factor_user'), 'error');
