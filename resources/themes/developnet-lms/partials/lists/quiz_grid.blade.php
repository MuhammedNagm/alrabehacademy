...
<div class="item subject-block">
  	<a href="{{$list_url}}" class="subject-img" @if(isset($open_tab)) target="_blank" @endif>
  		<img src="{{$quiz->thumbnail}}">
  		<span class="subject-read-more">@lang('developnet-lms::labels.links.link_read_more')</span>
  	</a>
  	<div class="subject-content">
			<a href="{{$list_url}}" class="sub-content-title" @if(isset($open_tab)) target="_blank" @endif>{{$quiz->title}}</a>
      @if($quiz->started_at)
  	...
  	<div class="subject-meta">
        ...
        <span style="font-weight: bold;">عدد الاسئلة :</span>  {{$quiz->questions()->count()}} | 

  			<i class="fa fa-group" title="المشتركين"></i>
