...
class QuizQuestionTransformer extends BaseTransformer
{
    ...
    public function transform(Question $question)
    {
        ...
        $quiz = Quiz::find(hashids_decode($quiz_hashed_id));

        $url = route($this->resource_url, ['quiz' => $quiz_hashed_id]);

        $actions = ['edit'=> [ 
            'href' => url($url . '/' . $question->hashed_id . '/edit'),
        ...
        if (user()->hasPermissionTo('LMS::question.delete')) {
            $actions['delete'] = [
                'icon'  => 'fa fa-fw',
                    'href' => url($url . '/' . $question->hashed_id),
        ...
        if($question->question_type == 'paragraph' && \Request::get('paragraph') != $question->hashed_id){
            $actions['questions'] =           [
           'href' => url($url . '?paragraph='.$question->hashed_id),
