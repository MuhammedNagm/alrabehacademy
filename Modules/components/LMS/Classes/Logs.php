...
class Logs
{
  ...
  //$attributes ['module', 'module_id', 'user' , 'parent' => ['type', 'id']];
  public function currentItemLog($attributes = [], $is_exercise = false)
  {
    ...
    if (!empty($attributes['parent'])) {
      ...
      if (!empty($parentLastLog)) {
        ...
        if ($itemLogs->count() > 1) {
          //              $itemLogsIds = $itemLogs->where('id', '!=', $itemLog->id)->pluck('id')->toArray();
          //              StudentLogs::whereIn('parent_id', $itemLogsIds)->delete();
          StudentLogs::where('user_id', $user->id)->where('id', '!=', $itemLog->id)->where('lms_loggable_type', $attributes['module'])->where('lms_loggable_id', $attributes['module_id'])->where('parent_id', $parentLastLog->id)->delete();
        }
      }
      ...
      if ($itemLogs->count() > 1) {
        //                 $itemLogsIds = $itemLogs->where('id', '!=', $itemLog->id)->pluck('id')->toArray();
        //                 StudentLogs::whereIn('parent_id', $itemLogsIds)->delete();
        StudentLogs::where('user_id', $user->id)->where('id', '!=', $itemLog->id)->where('lms_loggable_type', $attributes['module'])->where('lms_loggable_id', $attributes['module_id'])->where('parent_id', null)->delete();
      }
    }
  ...
  public function logsNumber($attributes = [], $parent = false)
  {
    ...
    if ($parent) {
      ...
      $number = $user->logs()->where('lms_loggable_type', $attributes['module'])
        ->where('lms_loggable_id', $attributes['module_id'])
        ->whereHas('parent', function ($q) use ($parent_id, $parent_type) {
    ...
    $number = $user->logs->where('lms_loggable_type', $attributes['module'])->where('lms_loggable_id', $attributes['module_id'])->where('parent_id', null)->count();
    return $number;
  }
  ...
  public function courseProgress($user, $course)
  {
    ...
    $courseLog = $this->currentItemLog([
      'user' => $user,
      'module' => 'course',
      'module_id' => $course ? $course->id : null,
      'parent' => []
    ]);
