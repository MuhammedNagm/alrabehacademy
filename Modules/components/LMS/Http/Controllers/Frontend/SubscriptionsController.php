...
class SubscriptionsController extends PublicBaseController
{
  ...
  public function store_subscription($request, $module, $module_data, $planned)
  {
    ...
    $not_free = true;
    if (!$module_data->coupons()->count()) {
      $not_free = false;
    }

    if ($module_data->show_in_plan > 0) {
      $not_free = true;
    }
    ...
    if (($not_free) || ($planned > 0)) {
      $rules = [
        'coupon' => 'required|min:3|max:14',
      ];
    } else {
      $rules = [];
    }
    ...
    if ($request->get('coupon')) {
      ...
      if (!$coupon || $coupon == '0') {
        return redirect()->back()->with(['message' => "Invalid Coupon", 'alert_type' => 'danger']);
      }
    ...
    if ($response['success']) {
      ...
      if ($request->get('coupon') && isset($coupon_array['coupon']) && $coupon_array['coupon']) {
        $couponData = $coupon_array['coupon'];
        if ($couponData->start_with_first_use && (int) $couponData->duration_in_days > 0) {
          $carbonNow = Carbon::now();
          $carbonNow2 = Carbon::now();
          $couponData->update(['start' => $carbonNow2->subDay(), 'expiry' => $carbonNow->addDays((int) $couponData->duration_in_days)]);
        }
      }
      if ($request->session()->has('lms_cart')) {
