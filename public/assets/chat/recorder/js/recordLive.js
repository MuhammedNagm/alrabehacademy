...
function changeVolume(value) {
 ...
 $('body').on('click','.start_recording',function(){

  var button = $(this);
  var stopButton = button.closest('.recorder-buttons-group').find('.stop_recording');
  var prevRecords = button.closest('.recorder-container').find('.recordings-list');
 recorder && recorder.record();
  button.attr('disabled', true);
  button.hide();
  stopButton.attr('disabled', false);
  stopButton.show();
  prevRecords.html('<tr><th id="sequencerBoxes"></th></tr>');
  console.log('Recording...');

  });

  $('body').on('click','.stop_recording',function(){

  var button = $(this);
  var startButton = button.closest('.recorder-buttons-group').find('.start_recording');
...
function handleWAV(blob) {
  ...
  // }
   ...
   toggler = document.createElement('input');
    ...
    $(toggler).attr('value', url);
    $(toggler).attr('name', 'audio_message');
    $(toggler).attr('link', downloadAnchor.download);
    newCell.appendChild(toggler);
}
