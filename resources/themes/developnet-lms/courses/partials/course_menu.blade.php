...
<li>  
	<div class="curriculum-section-header" data-toggle="collapse" href="#{{'collapseSection_'. $section->id}}" role="button" aria-expanded="false" aria-controls="{{'collapseSection_'. $section->id}}">
		<div><i class="fa fa-chevron-down"  @if($section->parent_id) style="color: #10a8dd;" @endif></i><h4  @if($section->parent_id) style="color: #10a8dd;" @endif>{{$section->title}}</h4></div>
	 	...
	 	<ul class="section-content" style="display: block;">
               ...
               $section_children =  $section->children()->orderBy('order', 'asc')->get();
                ...
                @if($section_children->count())
                    ...
                    @include('courses.partials.course_menu', ['section' =>  $child, 'courseSections' => $section_children,'user' => $authUser, 'course'=> $course])
                    @endforeach
	 		...
	 		<li class="course-item" style="{!! $bg_color !!} {!! $text_color !!}">
	 			...
	 			@if($row['type'] == 'quiz')
	 			<a class="lesson-title course-item-title button-load-item" href="{{route('quizzes.quizPage', ['quiz' => $row['hashId'], 'course' =>$course->hashed_id])}}" style="{!! $text_color !!}"> {{$row['title']}}</a>
