...
use Closure;
use Modules\Settings\Models\CustomField;
use ReflectionClass;
...
trait Hookable
{
    ...
    {
        if (method_exists($this, 'isCustomField') && $this->isCustomField($method)) {
            $customFieldModel = $this->customFieldRelation($method);
    ...
    public function __get($key)
    {
        ...
        if (method_exists($this, 'isCustomField') && $this->isCustomField($key)) {
            $customFieldModel = $this->customFieldRelation($key)->first();

            if ($customFieldModel instanceof CustomField && $customFieldModel->exists) {
