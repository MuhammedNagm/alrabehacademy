function initElements() {
    if (typeof CKEDITOR !== "undefined") {
        ...
        CKEDITOR.config.extraAllowedContent = 'span(*)'; //new
...
function handleAjaxSubmitError(response, textStatus, jqXHR, $form) {
    if (response.status === 422) {
        ...
        $.each(errors, function (field, message) {
            ...
            formGroup.addClass('has-error has-danger').append('<small class="help-block">' + message + '</small>');
        });
    }
    var data = {};
...
function switchFormFieldsTranslation(response, form, action_data) {
    ...
    form.data('action_data', null);
    form.data('page_action', null);
